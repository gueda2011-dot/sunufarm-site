  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-100 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-5">
        <Link href="/" className="text-lg font-bold tracking-tight text-green-700">
          SunuFarm
        </Link>

        <nav className="hidden items-center gap-6 text-sm text-gray-600 md:flex">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="transition-colors hover:text-gray-900">
              {l.label}
            </a>
          ))}
        </nav>
          ...
          <nav className="mt-3 flex flex-col gap-1">
            {links.map((l) => (
              <a
                ...
                className="py-2.5 text-sm font-medium text-gray-700 transition-colors hover:text-green-700"
              >
                {l.label}
              </a>
            ))}
